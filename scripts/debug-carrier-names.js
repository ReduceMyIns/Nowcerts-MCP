...
async function main() {
  try {
    ...
    for (let i = 0; i < carriers.length; i++) {
      const carrier = carriers[i];
      const name = carrier.name || carrier.insuredCommercialName || carrier.commercialName || carrier.contactName ||
    ...
    console.log();
    console.log('Carriers with names found: ' + carriers.filter(c => c.name || c.insuredCommercialName || c.commercialName || c.contactName || c.carrierName || c.companyName).length);
    console.log();
